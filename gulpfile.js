...
function fontTask(done) {
  src(config.src.fonts).pipe(dest(config.public.fonts));
  done();
}

function imagesTask(done) {
  src(config.src.images).pipe(dest(config.public.images));
  done();
}

function htmlTask(done) {
  src(config.src.html).pipe(dest(config.public.base));
  done();
}
...
exports.build = series(
  parallel(jsTask, cssTask, fontTask, imagesTask, htmlTask)
);
